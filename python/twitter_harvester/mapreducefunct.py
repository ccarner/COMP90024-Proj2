...
MAP_BY_SUBURB = '''function (doc) {
  emit([doc.sa2Code], doc.sentiment1, 1);
}'''


MAP_BY_WEEK2 = '''function (doc) {
  function getWeek(dateStr) {
    var date = new Date(dateStr);
    date.setHours(0, 0, 0, 0);
    // Thursday in current week decides the year.
  ...
  const week = getWeek(doc.time);
  const year = getWeekYear(doc.time);
  emit([doc.city, year, week], doc.sentiment2, 1);
}'''


MAP_BY_SUBURB2 = '''function (doc) {
  emit([doc.sa2Code], doc.sentiment2, 1);
}'''



# adapted from: https://docs.couchdb.org/en/latest/ddocs/ddocs.html
