...
def main(tweets = None):
    """The main function of the sentiment analysis. Identifies stopwords and 
    regex patterns and calls for the classifier to be trained. Optional 
    arguement of tweets in a list format to evaluate model accuracy."""
    stop_words = stopwords.words('english')
    # creates regex face pattern, faces are saved
    face_list = """:) :-) 8) =) :D :( :-( 8( =( (: (-: (8 (= D: ): )-: )8 )= :p""".split()
    face_regex = "|".join(map(re.escape, face_list))
    
    classifier = train_classifier(stop_words, face_regex)
    
    # If a tweet list is not given then terminate program
    if tweets == None:
        sys.exit("Classifier Created, No Tweets to Evaluate... Terminating")
        
    # Normalizes and tokenizes, lemmatizes and classifies tweets while 
    # Outputting each step, mainly used for diagnostic purposes
    for tweet in tweets:
        print("Tweet:                     " + tweet)
        tweet_tokens = tokenize(tweet, face_regex)
        print("Tokenized Tweet:           " + str(tweet_tokens))
        tweet_tokens_clean = token_clean(tweet_tokens, stop_words)
        print("Relevant Lemmatized Tweet: " + str(tweet_tokens_clean))
        sentiment = classifier.classify(dict([token, True] for token 
                                             in tweet_tokens_clean))
        print("Predicted Tweet Sentiment: " + str(sentiment))
        print('#############################################')

#main()
tweets = ['Today was a bad day!', 
          'It is great that McDonalds is open again...',
          'Lost my keys again today, long walk to the bus stop', 
          'Why are people so stupid',
          'Cannot wait until tonights movie!!!', 
          'My new website is up and running https://www.bestself.com.au',
          'I like this new smiley face :)', 
          '... some string with punctuation ...', 
          '@Test Will This Work?', 
          'finally!! this should be great $$$ (hopefully)', 
          'https://www.test.com.au is a great site!', 
          '@everyone just testing if this is working']
main(tweets)

"""
- CHANGE TO CORONAVIRUS DATA
- Get the average score of the classifer for what is positive and what is
  negative, this allows to see if people got firmer in beliefs over time
- Note FreqDist Might be useful in another setting. See V7 for relevant code
"""